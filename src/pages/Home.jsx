  ...
  return <main>
    <section className="home-hero shell">
      <div className="home-hero-copy">
        ...
        <div className="hero-ctas"><Link className="button primary" to="/testnet">Enter Testnet <ArrowRight size={16}/></Link><Link className="button secondary" to="/lcq">LCQ Consensus</Link><button className="button ghost" onClick={onConnect}><Wallet size={16}/>{walletState.account?'Open wallet':'Connect wallet'}</button></div>
    ...
    <section className="launch-center-section shell"><div className="launch-center-head"><div><span>RABBIT TESTNET · CHAIN ID 9280</span><h2>The public entry point for Rabbit.</h2></div><p>Wallet, RPC, explorer, faucet, mining and node access are organized around one official network hub.</p></div><div className="launch-grid"><LaunchAction icon={Wallet} label="WALLET" title="Connect wallet" text="Connect an installed EVM wallet without signing or giving custody." onClick={onConnect}/><LaunchAction icon={Network} label="NETWORK" title="Add Rabbit Testnet" text="Add Chain ID 9280 and the official RPC configuration directly to your wallet." onClick={()=>onAddNetwork(testnet)} disabled={!testnet.publicRpcReady} badge={testnet.publicRpcReady?'READY':'AT LAUNCH'}/><LaunchAction icon={FlaskConical} label="FAUCET" title="Get network assets" text="RAB and RUSD faucet access through the official Testnet service." to={testnet.faucetUrl} disabled={!testnet.publicFaucetReady} badge={testnet.publicFaucetReady?'OPEN':'COMING SOON'}/><LaunchAction icon={Globe2} label="EXPLORER" title="Inspect the chain" text="Blocks, transactions, addresses and contracts through the official explorer." to={testnet.explorerUrl} disabled={!testnet.publicExplorerReady} badge={testnet.publicExplorerReady?'OPEN':'COMING SOON'}/><LaunchAction icon={Cpu} label="MINING" title="Start mining" text="Download Rabbit, verify releases and participate through LCQ." to="/mining"/><LaunchAction icon={Network} label="NODE" title="Run a node" text="Operate independent P2P infrastructure and help propagate Rabbit." to="/nodes"/></div></section>
