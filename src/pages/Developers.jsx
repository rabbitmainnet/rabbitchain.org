...
export default function Developers() {
  ...
  return (
    <main>
      ...
      <section className="section shell" id="start"><SectionHeader eyebrow="DEVELOPER START" title="The shortest path from zero to Rabbit." text="Testnet is the default development environment before Mainnet opens." /><div className="developer-steps"><article><span>01</span><h3>Connect Testnet</h3><p>Use Chain ID 9280 and the official RPC after public activation.</p></article><article><span>02</span><h3>Fund a wallet</h3><p>Use tRAB from the faucet; RUSD is planned as an additional test asset.</p></article><article><span>03</span><h3>Deploy & call</h3><p>Use EVM contracts and familiar JSON-RPC transaction workflows.</p></article><article><span>04</span><h3>Inspect</h3><p>Confirm transactions and contracts through the Rabbit explorer.</p></article></div></section>
      ...
      <section className="section shell" id="networks"><SectionHeader eyebrow="NETWORK CONFIGURATION" title="Testnet first. Mainnet already reserved." text="Centralize Rabbit network parameters in your application and activate services only when they are public." /><div className="network-config-table"><div className="network-config-head"><span>Network</span><span>Chain ID</span><span>RPC</span><span>Explorer</span><span>Status</span></div><div><b>Rabbit Testnet</b><code>9280 / 0x2440</code><code>{t.rpcUrl}</code><code>{t.explorerUrl}</code><em>{t.networkLive ? 'LIVE' : 'PRE-LAUNCH'}</em></div><div><b>Rabbit Mainnet</b><code>928 / 0x3a0</code><code>{NETWORKS.mainnet.rpcUrl}</code><code>{NETWORKS.mainnet.explorerUrl}</code><em>{NETWORKS.mainnet.networkLive ? 'LIVE' : 'AFTER TESTNET'}</em></div></div></section>

      <section className="rpc-section" id="rpc"><div className="shell rpc-grid"><div><span className="section-kicker">JSON-RPC</span><h2>Use a standard Ethereum request.</h2><p>The request shape stays familiar. The endpoint remains clearly marked reserved until Testnet activation.</p><div className="rpc-endpoint"><code>{t.rpcUrl}</code><CopyButton value={t.rpcUrl} /></div></div><pre><code>curl -X POST {`\\`}{`\n`}  -H <em>'Content-Type: application/json'</em> {`\\`}{`\n`}  --data <em>'{'{'}"jsonrpc":"2.0",{`\n`}    "method":"eth_chainId",{`\n`}    "params":[],"id":1{'}'}'</em> {`\\`}{`\n`}  <b>{t.rpcUrl}</b></code></pre></div></section>
