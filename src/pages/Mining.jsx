...
const platforms = [
  ['Windows', 'x86_64', 'ZIP release', MonitorDown],
  ['Linux / Ubuntu', 'x86_64', 'tar.gz release', TerminalSquare],
  ['macOS', 'ARM64 / x86_64', 'release track', Cpu],
]

export default function Mining() {
  return (
    <main>
      <section className="page-hero mining-hero"><div className="shell page-hero-grid"><div className="page-hero-copy"><span className="hero-eyebrow"><i /> TESTNET MINING FIRST</span><h1>Download. Run. <em>Compete for opportunity.</em></h1><p>Rabbit mining is designed around permissionless participation: generate valid work and let LCQ coordinate block-production opportunities.</p><div className="hero-ctas"><a className="button primary" href="#downloads">Release track <ArrowRight size={15} /></a><Link className="button secondary" to="/lcq">How LCQ works</Link></div></div><div className="terminal-card"><div className="terminal-head"><span>rabbit-node</span><b>TESTNET PROFILE</b></div><pre><code><span>$</span> rabbit --network rabbit-testnet{`\n\n`}<b>✓</b> client initialized{`\n`}<b>✓</b> P2P discovery enabled{`\n`}<b>✓</b> LCQ consensus loaded{`\n`}<b>✓</b> participant wallet configured{`\n\n`}<em>public binary release pending_</em></code></pre></div></div></section>

      <section className="section shell" id="downloads"><SectionHeader eyebrow="OFFICIAL RELEASES" title="Downloads should be verifiable, not mysterious." text="Every public binary needs a version, platform, SHA-256 and an official release source." /><div className="download-grid">{platforms.map(([name, arch, type, Icon]) => <article key={name}><Icon size={22} /><span>{arch}</span><h3>{name}</h3><p>{type}</p><div className="download-meta"><small>VERSION</small><b>Pending public release</b><small>SHA-256</small><b>Published with release</b></div><button disabled><Download size={15} />Coming with Testnet</button></article>)}</div></section>
