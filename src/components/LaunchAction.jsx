...
export default function LaunchAction({ icon: Icon, label, title, text, to, onClick, disabled = false, badge }) {
  const content = <>
  ...
  if(disabled)return <div className="launch-action disabled" aria-disabled="true">{content}</div>
  if(onClick)return <button type="button" className="launch-action launch-action-button" onClick={onClick}>{content}</button>
