...
export default function WalletDrawer({ open, state, walletName, onClose, onDisconnect, onSwitch, toast }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={onClose}>
    <aside className="wallet-drawer" onMouseDown={(e)=>e.stopPropagation()}>
      ...
      <div className="wallet-identity-card"><div className="wallet-avatar"><img src="/rabbit-wallet-icon.png" alt=""/></div><div><b>{shortAddress(state.account)}</b><small>{walletName || 'Browser Wallet'}</small></div><button onClick={copyAddress}><Copy size={15}/></button></div>
